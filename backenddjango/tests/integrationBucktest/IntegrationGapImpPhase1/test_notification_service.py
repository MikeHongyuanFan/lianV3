...
@pytest.fixture
def application(admin_user, borrower_user, broker_user, bd_user):
    """Create a test application with borrower."""
...
@pytest.mark.django_db
def test_create_application_notification(application, admin_user, borrower_user, broker_user, bd_user):
    ...
    assert len(notifications) >= 4  # At least borrower, broker, BD, and admin
    ...
    assert db_notifications.count() >= 4
    
    # Verify notifications for each user type
    user_ids = [notification.user.id for notification in db_notifications]
    assert borrower_user.id in user_ids
    assert broker_user.id in user_ids
    assert bd_user.id in user_ids
